...
export class ProduitListAdminComponent extends AbstractListController<ProduitDto, ProduitCriteria, ProduitService>  implements OnInit {
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'prixVenteMin', header: 'Prix vente min'},
        ];
    }
    ...
    public async loadUniteMesure(){
        await this.roleService.findAll();
        const isPermistted = await this.roleService.isPermitted('Produit', 'list');
        isPermistted ? this.uniteMesureService.findAllOptimized().subscribe(uniteMesures => this.uniteMesures = uniteMesures,error=>console.log(error))
        : this.messageService.add({severity: 'error', summary: 'Erreur', detail: 'Problème de permission'});
    }
    public async loadMarque(){
        await this.roleService.findAll();
        const isPermistted = await this.roleService.isPermitted('Produit', 'list');
        isPermistted ? this.marqueService.findAllOptimized().subscribe(marques => this.marques = marques,error=>console.log(error))
        : this.messageService.add({severity: 'error', summary: 'Erreur', detail: 'Problème de permission'});
    }
   ...
   public prepareColumnExport() : void {
        ...
        //'Store': this.criteria.store?.libelle ? this.criteria.store?.libelle : environment.emptyForExport ,
            ...
            'Prix vente min Min': this.criteria.prixVenteMinMin ? this.criteria.prixVenteMinMin : environment.emptyForExport ,
            'Prix vente min Max': this.criteria.prixVenteMinMax ? this.criteria.prixVenteMinMax : environment.emptyForExport ,
        }];
      }
}
