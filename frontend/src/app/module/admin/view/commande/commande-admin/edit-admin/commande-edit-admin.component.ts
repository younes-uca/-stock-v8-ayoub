...
export class CommandeEditAdminComponent extends AbstractEditController<CommandeDto, CommandeCriteria, CommandeService>   implements OnInit {
   ...
   public async openCreatePaiementCommande(paiementCommande: string) {
        const isPermistted = await this.roleService.isPermitted('PaiementCommande', 'edit');
        if(isPermistted) {
             this.paiementCommande = new PaiementCommandeDto();
             this.createPaiementCommandeDialog = true;
        }else{
             this.messageService.add({
                severity: 'error', summary: 'erreur', detail: 'problème de permission'
            });
  ...
  set paiementCommande(value: PaiementCommandeDto) {
   ...
   get createPaiementCommandeDialog(): boolean {
       return this.paiementCommandeService.createDialog;
   }
  set createPaiementCommandeDialog(value: boolean) {
       this.paiementCommandeService.createDialog= value;
   }
