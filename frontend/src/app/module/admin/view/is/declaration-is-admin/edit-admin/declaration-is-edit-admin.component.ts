...
export class DeclarationIsEditAdminComponent extends AbstractEditController<DeclarationIsDto, DeclarationIsCriteria, DeclarationIsService>   implements OnInit {
   ...
   public async openCreateComptableTraitant(comptableTraitant: string) {
        const isPermistted = await this.roleService.isPermitted('ComptableTraitant', 'edit');
        if(isPermistted) {
             this.comptableTraitant = new ComptableTraitantDto();
   ...
   public async openCreateComptableValidateur(comptableValidateur: string) {
        const isPermistted = await this.roleService.isPermitted('ComptableValidateur', 'edit');
        if(isPermistted) {
             this.comptableValidateur = new ComptableValidateurDto();
