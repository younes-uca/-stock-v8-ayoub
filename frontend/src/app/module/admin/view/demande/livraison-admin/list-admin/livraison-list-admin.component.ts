...
export class LivraisonListAdminComponent extends AbstractListController<LivraisonDto, LivraisonCriteria, LivraisonService>  implements OnInit {
    ...
    constructor(livraisonService: LivraisonService, private demandeService: DemandeService, private etatLivraisonService: EtatLivraisonService, private clientService: ClientService) {
    ...
    public async loadEtatLivraison(){
        await this.roleService.findAll();
        const isPermistted = await this.roleService.isPermitted('Livraison', 'list');
        isPermistted ? this.etatLivraisonService.findAllOptimized().subscribe(etatLivraisons => this.etatLivraisons = etatLivraisons,error=>console.log(error))
        : this.messageService.add({severity: 'error', summary: 'Erreur', detail: 'Problème de permission'});
    }
